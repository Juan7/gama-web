...
if( isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) AND ($_POST['url']=='')){
	if (isset($_POST['name']) AND isset($_POST['email']) AND isset($_POST['message'])) {
		$to = 'gerencia@servicios-gama.com';  // Change it by your email address
    $subject='Contact from Identity';
		...
		if ($sent) {
			echo "<div class='content-message'> <i class='fa fa-rocket fa-3x'></i> <h2>Email Enviado Satisfactoriamente</h2> <p>Su mensaje ha sido enviado.</p> </div>";
		} else {
			echo "<div class='content-message'> <i class='fa fa-times fa-3x'></i> <h2>Error al enviar</h2> <p>Trate de nuevo porfavor.</p> </div>";
		}
	}
	else {
